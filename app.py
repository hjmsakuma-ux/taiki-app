...
# ★ パスワードはスプレッドシートから読み込むため、ここでは空にしておきます（後ほど関数で取得）
USER_CREDENTIALS = {}
...
def get_users_worksheet():
    try:
        gc = get_gc()
        # 「ユーザー管理」という名前のシートを開く
        return gc.open("待機表データ").worksheet("ユーザー管理")
    except:
        st.error("エラー: スプレッドシートに「ユーザー管理」シートが見つかりません。作成してください。")
        return None
...
def load_credentials_from_sheet():
    ...
    if ws:
        try:
            ...
            for r in records:
                # ユーザー名とパスワードを辞書にする
                creds[str(r['ユーザー名'])] = str(r['パスワード'])
            return creds
        except Exception as e:
            st.error(f"ユーザー情報の読み込みに失敗しました: {e}")
            return {}
    return {}
    ...
    for i, d in enumerate(dates):
        ...
        if duty_doc:
            ...
            if not is_fixed_assigned:
                ...
                if date_str in holiday_info:
                    ...
                    L = info["length"]
                    idx = info["index"]
                    if L >= 4: is_long_holiday_block = True
                    if L <= 3:
                        prefer_continue = True
                    elif 4 <= L <= 6:
                        if idx == (L // 2) + 1: force_change = True
                        else: prefer_continue = True
                    else:
                        p1 = (L // 3) + 1
                        p2 = 2 * (L // 3) + 1
                        if idx == p1 or idx == p2: force_change = True
                        else: prefer_continue = True
                ...
                for doc in DOCTORS:
                    ...
                    if doc == "三浦医師" and status != "HOPE":
                        if is_long_holiday_block: pass 
                        else:
                            valid_miura_day = False
                            if weekday == 4:
                                if miura_monthly_sets.get(current_ym, 0) == 0: valid_miura_day = True
                            elif weekday == 5:
                                if prev_doc == "三浦医師": valid_miura_day = True
                            elif weekday == 6:
                                if prev_doc == "三浦医師": valid_miura_day = True
                            if not valid_miura_day: continue

                    if not is_continuity_candidate:
                        if is_holiday and holiday_streak[doc] >= 3 and status != "HOPE": continue
                        if (is_sat or is_sun) and (doc in prev_weekend_docs) and status != "HOPE": continue
                        if is_sat and status != "HOPE":
                             past_sets = 0
                             for w in [1, 2, 3]:
                                 d_past = (dt - timedelta(weeks=w)).strftime('%Y-%m-%d')
                                 if schedule_result.get(d_past) == doc: past_sets += 1
                             if past_sets >= 2: continue
                        if is_sat and status != "HOPE":
                            if monthly_weekend_counts[current_ym][doc] >= 2: continue
                            if prev_ym in monthly_weekend_counts and monthly_weekend_counts[prev_ym][doc] >= 2:
                                if monthly_weekend_counts[current_ym][doc] >= 1: continue
                        if is_sat and status != "HOPE":
                            if weekend_counts[doc] >= 5: continue
                            elif weekend_counts[doc] >= 4: is_backup = True

                    if status == "HOPE": hope_candidates.append(doc)
                    else:
                        if is_backup: candidates_backup.append(doc)
                        else: candidates_normal.append(doc)

                def get_sort_key(doc_id):
                    if weekday == 4 and doc_id == "三浦医師" and miura_monthly_sets.get(current_ym, 0) == 0:
                        return -99999
                    if is_sat:
                        return (monthly_weekend_counts[current_ym][doc_id], weekend_counts[doc_id], get_weighted_count(doc_id, counts[doc_id]))
                    else:
                ...
                if hope_candidates:
                    ...
                    if winner == "人員不足":
                        final_candidates = []
                        if candidates_normal: final_candidates = candidates_normal
                        elif candidates_backup: final_candidates = candidates_backup
                        if final_candidates:
        ...
        if winner != "人員不足":
            ...
            if winner == "三浦医師" and weekday == 4:
                is_long = False
                if date_str in holiday_info and holiday_info[date_str]["length"] >= 4: is_long = True
                if not is_long: miura_monthly_sets[current_ym] += 1
            ...
            if is_holiday:
                ...
                for doc in DOCTORS:
                    if doc != winner: holiday_streak[doc] = 0
        else:
            schedule_result[date_str] = "人員不足"
            for doc in DOCTORS: holiday_streak[doc] = 0
...
def render_calendar_selector(year_months, period_label, current_year, doctor_name, login_user):
    ...
    with col_info:
        ...
        <div class="legend-box">
            ...
            <table style="width:100%; border-collapse:collapse; border:none;">
                <tr style="border:none;"><td style="border:none; text-align:center; font-weight:bold; font-size:1.5rem; width:40px;">〇</td><td style="border:none; text-align:center; width:20px;">：</td><td style="border:none; text-align:left;">待機希望</td></tr>
                <tr style="border:none;"><td style="border:none; text-align:center; font-weight:bold; font-size:1.5rem;">✖</td><td style="border:none; text-align:center;">：</td><td style="border:none; text-align:left;">待機不可</td></tr>
                <tr style="border:none;"><td style="border:none; text-align:center; font-weight:bold; font-size:1.5rem;">☆</td><td style="border:none; text-align:center;">：</td><td style="border:none; text-align:left;">当直</td></tr>
            </table>
    ...
    if (login_user in ADMIN_USERS or login_user == doctor_name) and not is_done:
        with st.expander("📆 期間を指定して一括入力", expanded=False):
            ...
            with c1: batch_start = st.date_input("開始日", min_date, min_value=min_date, max_value=max_date, key=f"b_start_{doctor_name}")
            with c2: batch_end = st.date_input("終了日", min_date, min_value=min_date, max_value=max_date, key=f"b_end_{doctor_name}")
            with c3: batch_type = st.selectbox("設定内容", ["〇 待機希望", "✖ 待機不可", "☆ 当直", "⚪ 解除"], key=f"b_type_{doctor_name}")
            with c4:
                st.write("") 
                st.write("") 
                if st.button("一括反映", key=f"b_btn_{doctor_name}", type="primary"):
                    if batch_start > batch_end: st.error("開始日が終了日より後になっています")
                    else:
            ...
            for i, day in enumerate(week):
                ...
                if login_user in ADMIN_USERS or login_user == doctor_name:
                    ...
                    with popover:
                        ...
                        st.button("〇 待機希望", key=f"hope_{key}", on_click=update_pref, args=(doctor_name, date_str, "HOPE"), use_container_width=True)
                        st.button("✖ 待機不可", key=f"ng_{key}", on_click=update_pref, args=(doctor_name, date_str, "NG"), use_container_width=True)
                        st.button("☆ 当直", key=f"duty_{key}", on_click=update_pref, args=(doctor_name, date_str, "当直"), use_container_width=True)
                        st.button("⚪ 解除", key=f"clr_{key}", on_click=update_pref, args=(doctor_name, date_str, None), use_container_width=True)
                else:
    ...
    for target_y, target_m in months_to_render:
        ...
        for week in cal:
            ...
            for i, day in enumerate(week):
                ...
                is_holiday = check_is_holiday(date_obj)
                day_label = str(day)
                if is_holiday: day_label = f"{day}<span style='color: #e60000; font-size: 0.9em;'>(祝)</span>"
...
def render_admin_manual_page():
    ...
    if USER_CREDENTIALS:
        ...
        st.info("スプレッドシートからユーザー情報を読み込めませんでした。")
...
with st.sidebar.expander("🔑 パスワード変更"):
    new_pass = st.text_input("新しいパスワード", type="password", key="new_pass_input")
    confirm_pass = st.text_input("確認用", type="password", key="confirm_pass_input")
    
    if st.button("変更する"):
        if new_pass and new_pass == confirm_pass:
            if update_password_in_sheet(login_user, new_pass):
                st.success("パスワードを変更しました！")
                st.info("次回ログイン時から有効です。")
        else:
            st.error("パスワードが一致しないか、空欄です。")
